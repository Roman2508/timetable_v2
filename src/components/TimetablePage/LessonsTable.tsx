...
const LessonsTable: React.FC<ILessonsTable> = ({
  ...
  const { groupLoad, loadingStatus } = useSelector(scheduleLessonsSelector)

  React.useEffect(() => {
    if (!selectedItemId) return

    if (scheduleType === "group") {
      dispatch(findLessonsForSchedule({ semester: selectedSemester, groupId: selectedItemId }))
    }
  ...
  return (
    <Table>
      ...
      <TableBody>
        ...
        {(!groupLoad || !groupLoad.length) && loadingStatus !== LoadingStatusTypes.LOADING && (
          <TableRow style={{ textAlign: "center" }}>
            <TableCell colSpan={5}>
              <EmptyCard />
            </TableCell>
